...
import { getStarkEntry } from '../data/stark_database'; // RESTORED
               ...
               <tbody className="divide-y divide-white/5">
                 {displayedData.map((line, idx) => (
                   <React.Fragment key={idx}>
                    ...
                    >
                      <td className="p-4 text-white font-bold tracking-wider">
                        {line.wavelength.toFixed(3)}
                        {getStarkEntry(line.element, line.ion, line.wavelength) && ( // RESTORED
                          <span className="ml-2 px-1 py-0.5 border border-[#00f0ff]/30 text-[#00f0ff] text-[8px] bg-[#00f0ff]/10 rounded font-bold" title="Stark broadening nₑ data available">⚡ nₑ</span>
                        )}
                      </td>
